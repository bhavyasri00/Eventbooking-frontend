        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {loading ? (
            ...
            filteredEvents.map((event) => (
              <div
                key={event.id}
                className="bg-opacity-80 backdrop-blur rounded-lg overflow-hidden shadow-md hover:shadow-xl transition-shadow duration-300 border border-gray-100"
              >
                {/* Event Image */}
                <div className="w-full h-56 bg-gray-200 overflow-hidden">
                  <img
                    src={
                      event.image
                        ? (() => {
                            // If already full HTTP URL, use as-is
                            if (event.image.startsWith("http")) {
                              return event.image;
                            }
                            // If it's a path starting with /, use localhost:5000
                            if (event.image.startsWith("/")) {
                              return `http://localhost:5000${event.image}`;
                            }
                            // If it's a filename, prepend uploads path
