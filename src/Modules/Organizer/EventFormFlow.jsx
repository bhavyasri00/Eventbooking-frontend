        ...
        <div className="p-4 border-2 border-gray-300 rounded-lg hover:border-red-500 cursor-pointer transition">
          <div className="flex items-center gap-3">
            <input
              ...
              id="full-refund"
              value="full"
              checked={form.refundPolicy === "full"}
              onChange={(e) =>
                setForm((prev) => ({
                  ...prev,
        ...
        <div className="p-4 border-2 border-gray-300 rounded-lg hover:border-red-500 cursor-pointer transition">
          <div className="flex items-center gap-3">
            <input
              ...
              id="partial-refund"
              value="partial"
              checked={form.refundPolicy === "partial"}
              onChange={(e) =>
                setForm((prev) => ({
                  ...prev,
                  refundPolicy: e.target.value,
        ...
        <div className="p-4 border-2 border-gray-300 rounded-lg hover:border-red-500 cursor-pointer transition">
          <div className="flex items-center gap-3">
            <input
              ...
              id="no-refund"
              value="none"
              checked={form.refundPolicy === "none"}
              onChange={(e) =>
                setForm((prev) => ({
                  ...prev,
                  refundPolicy: e.target.value,
...
export default function EventFormFlow({ ev = null, onCancel, onSave }) {
  ...
  const [form, setForm] = useState(() =>
    ev
      ...
      : {
          ...
          refundPolicy: "full", // Added refund policy
          ticketsSold: 0,
          totalPrice: 0,
  ...
  const validateStep1 = () => {
    ...
    if (!form.time || !String(form.time).trim())
      newErrors.time = "Time is required";
    if (!form.venue || !String(form.venue).trim())
      newErrors.venue = "Venue is required";
    if (!form.category || !String(form.category).trim())
      newErrors.category = "Category is required";
    setErrors(newErrors);
    const isValid = Object.keys(newErrors).length === 0;
    console.log("validateStep1 result:", { isValid, form, newErrors });
    return isValid;
  };
  ...
  const handleNext = () => {
    if (step === 1) {
      if (!validateStep1()) {
        console.warn("Validation failed for step 1:", errors);
        return;
      }
    }
  ...
  // Handle image upload and event submission
  const handleSaveWithImage = async () => {
    try {
      let imagePath = form.imagePath; // in case already uploaded
      ...
      if (
        ...
        // Only upload if it's a data URL (new image selected)
        ...
        if (uploadData.path) {
          imagePath = uploadData.path; // URL returned from backend
        }
      }
      ...
      const eventData = {
        ...form,
        date: utcDateTime.toISOString().slice(0, 19),
        image: imagePath,
      };

      onSave(eventData); // send to backend
    } catch (error) {
  ...
  return (
    <div>
      ...
      <div className="bg-white p-6 rounded-lg border border-gray-200 min-h-[400px]">
        ...
        {step === 2 && <Step5 form={form} setForm={setForm} />}
      </div>
